...
<ul&gt<li&gtManual or Z-Wave on/off, brightness, and RGBW color control</li&gt <li&gtPerfect for programming your existing RGBW LED strip</li&gt <li&gtEnergy monitoring in live mode and over time</li&gt <li&gtDouble tap to full brightness feature + optional scene control</li&gt <li&gtRemembers and restores on/off status a?er power failure</li&gt <li&gtBuilt-in Z-Wave Plus signal repeater to extend network range</li&gt </ul&gt

### Inclusion Information

<p&gtMake sure the RGBW Dimmer is powered up. <strong&gtClick the Z-Wave button 3 times </strong&gt<strong&gtquickly</strong&gt to include the device. The LED will blink yellow to indicate communication with the hub. It will blink green (for non-secure, S0, or S2 unauthenticated inclusion) or magenta (for S2 authenticated) once the device is successfully included to your network. It will blink red if inclusion is not successful</p&gt

### Exclusion Information

<p&gtClick the Z-Wave button 3 times quickly.</p&gt
...
### Preset Program
<p&gtAllowed values are 0, 6, 7, 8, 9 and 10.</p&gt
...
<p&gtAllowed values: 0 (disabled) or 30 - 32400 seconds</p&gt <p&gtDefault: 3600 seconds (1 hour)</p&gt
...
<p&gtChoose how you want your RGBW Dimmer to report voltage changes from the connected analog sensors and trigger status change on the output. The number entered as value corresponds to decimal values in voltage change. So if 5 is entered by default, the RGBW Dimmer will report any change in voltage of the connected sensor over 0.5 V. This setting is only in effect if at least one of Parameters 20 – 23 is set to value 0 or 1 (input is set to analog sensor mode).</p&gt <p&gtAllowed range: 0 -100</p&gt <p&gtDefault: 5</p&gt
...
<p&gtChoose how often you want your RGBW Dimmer to report voltage changes from the connected analog sensors to your hub. The number entered as value corresponds to the number of seconds. So if 3600 isentered as value, the RGBW Dimmer will report voltage from connected analog sensors every hour (independent of the threshold set in parameter 63). Voltage reports are sent in at least 30-second intervals. This setting is only in effect if at least one of Parameters 20 – 23 is set to value 0 or 1 (input is set to analog sensor mode).</p&gt <p&gtAllowed values: 0 (disabled) or 30 - 32400 seconds</p&gt <p&gtDefault: 0 (disabled)</p&gt
The following option values may be configured, in addition to values in the range 30 to 32400 -:
...
<p&gtChoose how you want your RGBW Dimmer to report energy use (kWh) to your hub. The number entered as value corresponds to decimal values in energy usage change. So if 10 is entered by default, the RGBW Dimmer will report any change in energy use over 0.1 kWh. Energy reports are sent in at least 30-second intervals.</p&gt <p&gtAllowed values: 0 (disabled) or 1 - 500 (0.01 - 5.00 kWh)</p&gt <p&gtDefault: 10 (0.1 kWh)</p&gt
...
<p&gtChoose how often you want your RGBW Dimmer to report energy (kWh) to your controller. The number entered as value corresponds to the number of seconds. So if 3600 is entered by default, the RGBW Dimmer will report energy use every hour(independent of the threshold set in parameter 65). Energy reports are sent in at least 30-second intervals.</p&gt <p&gtAllowed values: 0 (disabled) or 30 - 32400 seconds</p&gt <p&gtDefault: 3600 seconds (1 hour)</p&gt
The following option values may be configured, in addition to values in the range 30 to 32400 -:
...
<p&gtChoose how switches connected to the input terminals control your LED strip. In the default RGBW mode, each switch connected to each input terminal controls the RGBW channels separately (so IN1 for red, IN2 for green, etc) with a single click for ON/OFF, double click to full brightness, and press-and-hold to dim (momentary switches only). In the HSB mode, IN1 controls hue, IN2 – saturation, IN3 – brightness, IN4 – white with single click for last set value or value 0, double click to max value, and press-and-hold to set custom value.</p&gt
The following option values may be configured -:
...
<p&gtAdjust the ramp rate for your RGBW dimmer (fade-in/fade-out effect for ON/OFF operation). Values correspond to the number of seconds it takes for the dimmer to reach full brightness or turn OFF when operated from the connected wall switch.</p&gt <p&gtYou can also set the physical ramp rate in minutes instead of seconds by using a value between 128-254. For minutes, subtract 127 from the value (e.g. a value of 132 corresponds to 132 - 127 = 5 minutes).</p&gt <p&gtAllowed values: 0 (instant ON/OFF), 1 - 127 seconds, or 128 - 254 (1 - 127 minutes)</p&gt
...
<p&gtAdjust the ramp rate for your RGBW dimmer (fade-in / fade-out effect for on / off operation). Values correspond to the number of seconds it take for the dimmer to reach full brightness or turn off when operated from the connected wall switch.</p&gt <p&gtYou can also set the Z-Wave ramp rate in minutes instead of seconds by using a value between 128-254. For minutes, subtract 127 from the value (e.g. a value of 132 corresponds to 132 - 127 = 5 minutes).</p&gt <p&gtAllowed values: 0 instant ON/OFF), 1 - 127 seconds, or 128 - 254 (1 - 127 minutes)</p&gt
Values in the range 0 to 254 may be set.
...
<p&gtSet the status or brightness level triggering other associated devices in your network if the dimmer is turned ON with a single click. Each of the 4 available bytes refers to one channel, starting with IN1 and ending with IN4. This setting applies to basic set and switch multilevel commands sent to associated devices in the appropriate association group.</p&gt <p&gtAllow value: 0 - 99, 255 (ON)</p&gt
...
<p&gt0 - Preset programs disabled</p&gt <p&gt6 - Fireplace</p&gt <p&gt7 - Storm</p&gt <p&gt8 - Rainbow</p&gt <p&gt9 - Polar lights</p&gt <p&gt10 - Police (strobing red, white and blue)</p&gt
The following option values may be configured -:
...
<p&gtReserved for association and sync with other Zooz RGBW Dimmers (not for use with other devices).</p&gt
...
<p&gtON/OFF operation sync from input IN1</p&gt
...
<p&gtBrightness (dimming) sync from input IN1</p&gt
...
<p&gtON/OFF operation sync from input IN2</p&gt
...
<p&gtBrightness (dimming) sync from input IN2</p&gt
...
<p&gtON/OFF operation sync from input IN3</p&gt
...
<p&gtBrightness (dimming) sync from input IN3</p&gt
...
<p&gtON/OFF operation sync from input IN4</p&gt
...
<p&gtBrightness (dimming) sync from input IN4</p&gt
