...
<p&gtThe Double Plug works great as a low profile on/off device for a couple of lamps or small appliances (10A total between both outlets). But it can serve as a smart power monitor, a signal tester, or extender as well. It's fast, it's secure, and it's small.</p&gt <p&gtAs always, we've included a bunch of useful settings for you to customize the plug so you get the most of it. There is a built-in timer for remote reboots of connected devices or the option to turn off manual control. But if you find anything missing, just let us know and we'll add it</p&gt

### Inclusion Information

<p&gtAUTO INCLUSION</p&gt <ol&gt<li&gtInitiate inclusion (pairing) in the app (or web interface). Not sure how? ask@getzooz.com</li&gt <li&gtPlug the device into a grounded 120 VAC receptacle you’re planning to use it at (it doesn’t need to be close. Put your Z-Wave hub into inclusion mode and click the Z-Wave button 3 times quickly. The LED indicators will start blinking and the Double Plug will join the network automatically. If it lights up in yellow, inclusion has failed.</li&gt <li&gtYou will see 4 new on/off devices in your app: 2 channels for on/off control, 1 channel for on/off monitoring (the USB port reporting charging/ charged status, no control), and 1 master node to control both outlets at the same time and to access and adjust advanced settings in (if your hub supports parameter input).</li&gt </ol&gt<p&gtMANUAL INCLUSION</p&gt <p&gtPut your Z-Wave hub into inclusion mode and click the Z-Wave button 3 times quickly.</p&gt

### Exclusion Information

<ol&gt<li&gtBring the plug within direct range of your Z-Wave gateway (hub).</li&gt <li&gtPut the Z-Wave hub into exclusion mode (not sure how to do that? ask@getzooz.com).</li&gt <li&gtPress and release the Z-Wave button 3 times quickly.</li&gt <li&gtYour hub will confirm exclusion and all of the channels (child outlets) and the main device will disappear from your controller's device list.</li&gt </ol&gt

### General Usage Information

<h1&gtRange Test Tool</h1&gt <p&gtYou can easily check if the Double Plug is within your Z-Wave hub’s range:</p&gt <p&gtPress and hold the Z-Wave button for 6 to 8 seconds until the LED indicator turns violet. The LED indicator will then report signal strength.</p&gt <ul&gt<li&gt<strong&gtSolid green</strong&gt = direct communication with the primary controller is stable</li&gt <li&gt<strong&gtFlashing green</strong&gt = direct communication with the primary controller is established but signal is weak</li&gt <li&gt<strong&gtSolid orange</strong&gt = stable communication with hub via repeaters</li&gt <li&gt<strong&gtFlashing orange</strong&gt = communication with the primary controller is established via repeaters but signal is weak</li&gt <li&gt<strong&gtSolid red</strong&gt = communication with the primary controller has failed</li&gt </ul&gt<p&gtPress and release the Z-Wave button to exit testing mode.</p&gt <p&gt<strong&gtNOTE</strong&gt: This function may only be activated once the Double Plug has been included to a Z-Wave network.</p&gt
...
<p&gtChoose the recovery state for your Double Plug’s outlets if power outage occurs.</p&gt
...
<p&gtThe number entered as value corresponds to the number of seconds. So if 30 is entered by default, the Double Plug will report power consumption (Wattage) every 30 seconds.</p&gt
...
<p&gtChoose how often you want your Double Plug to report power consumption (W) to your controller (hub). The number entered as value corresponds to the number of seconds. So if 300 is entered by default, the Double Plug will report energy usage every 300 seconds (5 minutes).</p&gt
Values in the range 30 to 2678400 may be set.
...
<p&gtChoose how often you want your Double Plug to report voltage to your controller (hub). The number entered as value corresponds to the number of seconds. So if 300 is entered by default, the Double Plug will report voltage every 300 seconds (5 minutes).</p&gt
Values in the range 30 to 2678400 may be set.
...
<p&gtChoose how often you want your Double Plug to report electrical current (A) to your controller (hub). The number entered as value corresponds to the number of seconds. So if 300 is entered by default, the Double Plug will report current every 300 seconds (5 minutes).</p&gt
Values in the range 30 to 2678400 may be set.
...
Overload Protection
<p&gtSet the amp level for overload protection to kick in. The number entered as the value corresponds to the total number of amps the Double Plug needs to detect across both outlets to turn off automatically and send a notification to the hub and associated devices.</p&gt
...
<p&gtUse this parameter to enable or disable the auto turn-off timer for the left (CH1) outlet. If this feature is enabled, the device connected to CH1 outlet will automatically turn off after a fixed period of time from being turned on (set in parameter 9).</p&gt
The following option values may be configured -:
...
<p&gtUse this parameter to set the time after which you want the device connected to the left (CH1) outlet to automatically turn off once it has been turned on. The number entered as value corresponds to the number of minutes.</p&gt
...
<p&gtUse this parameter to enable or disable the auto turn-on timer for the left (CH1) outlet. If this feature is enabled, the device connected to CH1 outlet will automatically turn on after a fixed period of time from being turned off (set in parameter 11).</p&gt
The following option values may be configured -:
...
<p&gtUse this parameter to set the time after which you want the device connected to the left (CH1) outlet to automatically turn on once it has been turned off. The number entered as value corresponds to the number of minutes.</p&gt
Values in the range 1 to 65535 may be set.
...
<p&gtUse this parameter to enable or disable the auto turn-off timer for the right (CH2) outlet. If this feature is enabled, the device connected to CH2 outlet will automatically turn off after a fixed period of time from being turned on (set in parameter 13).</p&gt
The following option values may be configured -:
...
<p&gtUse this parameter to set the time after which you want the device connected to the right (CH2) outlet to automatically turn off once it has been turned on. The number entered as value corresponds to the number of minutes.</p&gt
Values in the range 1 to 65535 may be set.
...
<p&gtUse this parameter to enable or disable the auto turn-on timer for the right (CH2) outlet. If this feature is enabled, the device connected to CH2 outlet will automatically turn on after a fixed period of time from being turned off (set in parameter 15).</p&gt
The following option values may be configured -:
...
<p&gtChoose if you want to use the Z-Wave button on the Double Plug to turn the outlets on or off manually or if you want to disable this function. If this parameter is set to 0 (disabled), you will only be able to turn any of the outlets on or off remotely using your Z-Wave gateway controller.</p&gt
The following option values may be configured -:
...
LED Indicator Mode
<p&gtChoose if you want the LED indicators next to each outlet to come on when power is on or off, or if you want to disable them.</p&gt
The following option values may be configured -:
...
<p&gtChoose if you want the plug to report power use or on/oﬀ activity in the USB port to the hub.</p&gt
The following option values may be configured -:
