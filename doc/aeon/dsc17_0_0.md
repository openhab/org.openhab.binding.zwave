...
<p&gtThe Aeon Labs Micro Double SES is a power binary switch device based on Z-wave routing slave library V5.02 Patch3</p&gt

### Inclusion Information

<p&gtToggle external button or push internal button.</p&gt

### Exclusion Information

<p&gtToggle external button 10 times in quick succession to initiate removing from the Z-Wave network or push internal button. </p&gt
...
<p&gtThis is a double byte value. The LSB defines the total time the device need to blink. The value is set in seconds. The MSB defines the on/off interval of the blinking. The unit is 0.1 s.</p&gt <p&gtConfiguration Value 1 is to Specify the time that Micro Double SES need blink, The unit is Second; Configuration Value 2 is to Specify the Cycle of on/off, the unit of it is 0.1 second. For example: if we set Configuration Value 1 to '15',Configuration Value 2 to '10',then Micro Double SES will open 0.5 second, close 0.5 second, and repeat for 14 times. </p&gt
...
<p&gtThe value is a combination number made up of the sum of the values for the individual parameters. The parameters have the values: KWH=8, Wattage=4, Current=2, Voltage=1.</p&gt
