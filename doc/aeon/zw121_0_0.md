...
<p&gtAeotec LED Strip has been crafted to power connected lighting using Z-Wave Plus. It is powered by Aeotec’s Gen5 technology. The LED Strip is a multi-colored LED Strip which allows control (on/off/dim/color change) via Z-Wave commands.</p&gt <p&gtNote that configuration parameters 37 and 39 only apply when the strip is in color transition mode. Color transition mode is entered when a change to parameter 37 or 39 is submitted. Color transition mode is exited when a change to any of the channels is submitted. </p&gt

### Inclusion Information

<ol&gt<li&gtSet your Z-Wave controller into pairing mode.</li&gt <li&gtPress the Action Button located on the back of the LED Strip's white controller box once (The network LED will blink green as an indication it is pairing insecurely, please refer to section "Enabling Security Encryption" in the Advanced Function section of the manual to pair it securely).</li&gt <li&gtIf the LED Strip has been successfully added to your Z-Wave network, its Network LED will become solid. If the pairing was unsuccessful, the red LED will be on for 2 seconds and then remain a colorful gradient, repeat the instructions above from step 1. </li&gt </ol&gt

### Exclusion Information

<ol&gt<li&gtSet your Z-Wave controller into device removal mode.</li&gt <li&gtPress the Action Button located on the back of the LED Strip's white controller box once.</li&gt <li&gtIf the LED Strip has been successfully removed from your  Z-Wave network, its Network LED will remain colorful gradient. If the removal was unsuccessful, the Network LED will still be solid, repeat the instructions above from step 1.</li&gt </ol&gt
...
<p&gtEnable/disable to turn on the last brightness level of the LED Strip when using the Color Switch Set CC to change its color.</p&gt <p&gt0=disable.</p&gt <p&gt1=enable.</p&gt <p&gtOthers=ignore. </p&gt
...
<p&gtReboot/save/exit Colorful mode.</p&gt <p&gt0=Un-reboot Colorful mode.</p&gt <p&gt1=Reboot Colorful mode.</p&gt <p&gt2=Exit Colorful mode.</p&gt <p&gt3=Save the current Colorful mode value and then to be exited. </p&gt
The following option values may be configured -:
...
<p&gtValue 0 means constant speed. Value 1-30 sets the speed to accelerate/decelerate. </p&gt
...
Applies only when the Color Transition Style is set to Fade Out/In
<p&gtAmount of time spent to transition between colors.</p&gt
The following option values may be configured -:
...
<p&gtWhen the end is reached the strip turns off.</p&gt
...
<p&gtNote that this setting only affects the brightness when in color transition mode. Use the "Brightness" channel to set the brightness outside of color transition mode.</p&gt
...
<p&gtNote that "Single Color Mode" does not select a constant single color (as the "Color Control" channel does), but instead cycles through a single color. </p&gt
The following option values may be configured -:
...
<p&gtNote that "Smooth Color Transition" will lead to invisible transitions when "Color Display Cycle" is set to "Single Color Mode".</p&gt
The following option values may be configured -:
...
<p&gtColorful mode configuration.</p&gt <p&gt1 = Rainbow mode.</p&gt <p&gt2 = Multi color mode.</p&gt <p&gt3 = Fade out and fade in (Red).</p&gt <p&gt4 = Fade out and fade in (Green).</p&gt <p&gt5 = Fade out and fade in (Blue).</p&gt
The following option values may be configured -:
...
Dimmer mode:
<p&gtDimmer mode:</p&gt <p&gt0 = Parabolic curve.</p&gt <p&gt1 = Index curve.</p&gt <p&gt2 = (Parabolic + Index)/2.</p&gt <p&gt3 = Linear. </p&gt
The following option values may be configured -:
...
<p&gt Use configuration parameter 80 to define what notifications this group receives.</p&gt  
