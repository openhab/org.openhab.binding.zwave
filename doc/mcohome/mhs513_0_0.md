...
<p&gtINCLUDING the device into Z-Wave network:</p&gt <p&gt1. Set controller into INCLUSION mode. Follow instructions provided by controller manufacturer.</p&gt <p&gt2. Press and hold any key on the panel for 8 seconds or 3 clicks in quick succession.</p&gt <p&gtNote: If inclusion is successful, all INDICATION LEDs on the panel will flash 4 times. </p&gt

### Exclusion Information

<p&gtEXCLUDING the device from Z-Wave network:</p&gt <p&gt1. Set controller into EXCLUSION mode. Follow instructions provided by controller manufacturer.</p&gt <p&gt2. Press and hold any key on the panel for 8 seconds or 3 clicks in quick succession.</p&gt <p&gtNote: If the exclusion is successful, all INDICATION LEDs on the panel will flash 4 times. The exclusion will delete all association data.</p&gt
...
ALL ON/ALL OFF
<table&gt<tr&gt<td&gt <p&gt0=forbid ALL ON, forbid ALL OFF</p&gt <p&gt1=forbid ALL ON, allow ALL OFF</p&gt <p&gt2=allow ALL ON, forbid ALL OFF</p&gt <p&gt255=allow ALL ON, allow ALL OFF</p&gt </td&gt </tr&gt</table&gt
...
<p&gt0 LED disabled 1-10 Min level-Max level >10 same level as 10</p&gt
...
Key Mode
<p&gt0=single click to switch on/off status</p&gt <p&gt1=Key default as off state. When it is turned on, then it will be turned off automatically after a time period , which can be set in item 5</p&gt <p&gt2=Key default as on state. When it is turned off, then it will be turned on automatically after a time period , which can be set in item 5</p&gt <p&gt3=hold >3s then key is on, and off once released</p&gt <p&gt4=single click to switch on/off status +hold >3s then key is on, and off once released</p&gt
...
Key On duration
<p&gt0x00 infinite</p&gt <p&gt1~65535 unit "sec"</p&gt
...
<p&gtSend unsolicited reports to the gateway</p&gt
...
<p&gtsend Basic Set Command to associated devices</p&gt
