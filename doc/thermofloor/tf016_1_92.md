...
<p&gtThe Heatit Z-Wave is designed for controlling electrical heating cables and electrical radiators/convectors. The thermostat has 2 internal temperature sensors and has 3 algorithms for temperature compensation. After you have set the temperature, the thermostat will use a hysteresis to regulate the temperature. The hysteresis is adjustable.</p&gt <p&gtThe temperature shown in the display is the actual temperature for floor sensor and the external room sensor. You are able to calibrate the sensor values. When using the internal room sensor, the temperature shown is the set point. When using the internal sensor, the thermostat can seem somewhat inaccurate. This is due to heat generation inside the thermostat, draft and other external influences.</p&gt <p&gt1.92 is an unofficial firmware upgrade with the following changes:</p&gt <p&gt\* Reporting of the relay status is fixed.</p&gt <p&gt\* All temperature sensors can be reported (internal, floor, external) via its association groups</p&gt
...
<p&gtSee information for inclusion.</p&gt
...
### Switch
<p&gtThis switch turns off the device. Under thermostat control, this switch will be on at all times, and the other switch channel will report whether the heating is currently active.</p&gt
...
### Power setting value
<p&gtIf device is in P (Power) mode, this value will give the fraction of time the heating is active as a number between 0 and 10. </p&gt
...
### Relay state
<p&gtThis switch reports the current state of the relay in the unit. </p&gt
...
<p&gtInternally measured air thermometer. Use value with care, the thermometer is affected by internal heating in the device, and there is firmware in the unit trying to correct for internal heating.</p&gt
...
<p&gtTemperature as measured by an an external thermometer connected to the device. Zero if no extra thermometer is wired up.</p&gt
...
<p&gtTemperature measured by a floor temperature sensor wired to the device. Zero if no sensor is connected.</p&gt
...
<table&gt<tr&gt<td&gtDefines Operation Mode of thermostat</td&gt </tr&gt</table&gt<p&gt0....OFF(0x00)</p&gt <p&gt1.....HEAT (0x01)</p&gt <p&gt2.....COOL (0x02)</p&gt <p&gt11....ENERGY SAVE HEAT (0x0B)</p&gt <p&gtEnergy Save heat is a mode with larger hystersis.</p&gt
...
<p&gtSet this according to the actual floor sensor that is attached to the thermostat.</p&gt
The following option values may be configured -:
...
<p&gtIn this mode you are able to make changes to the hysteresis in the thermostat. This means that you can changes the hysteresis from 0.2°C up to max 3.0°C.  Default setting is 0.5°C. When using a waterbased heating, the recommended hysteresis is a minimum of 1.0°C.</p&gt
...
<p&gtWhen heating is activated, ON is sent to this channel, and when turned off (due to reaching setpoint), OFF will be sent.</p&gt
