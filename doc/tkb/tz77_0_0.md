...
<p&gt12V DC operating voltage.</p&gt <p&gtThis insert switch module is a transceiver which is a Z-Wave enabled device<br /&gtand is fully compatible with any Z-Wave enabled network. <br /&gtRemote on/off control of the connected load is possible with other manufacturer's<br /&gtwireless controller. Each switch is designed to act as a repeater.</p&gt
...
<p&gtDefault status of socket: LED is on as indicator when the load is off</p&gt
...
<p&gtDefault with memory: the socket status is same as before when power on</p&gt
Values in the range 0 to 1 may be set.
