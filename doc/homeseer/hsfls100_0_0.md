...
<p&gtThe HS-FLS100+ is a PIR module that’s designed to retrofit onto existing outdoor floodlights. It features a PIR sensor to detect motion through movement of heat sources and a LUX sensor for determining brightness of its surroundings. It is equipped with Z-Wave Plus wireless communication capability that will convert an ordinary motion-activated floodlight into a smart home device.</p&gt <p&gt<strong&gtConventional Floodlight Mode</strong&gt HS-FLS100+ can be used as a conventional motion-activated floodlight sensor to power floodlights when motion is sensed during dark hours. Controls are provided for manual adjustment of LUX sensitivity and floodlight ON times.</p&gt <p&gt<strong&gtSmart Floodlight Mode</strong&gt When added to a smart home system, HS-FLS100+ will send Z-Wave commands to the smart hub or home controller when motion is sensed and when the LUX level changes. Likewise, the smart hub or home controller can send Z-Wave commands back to the HS-FLS100+ to turn floodlights ON and OFF. This added functionality provides a very high level of flexibility not typically available in conventional motion-activated floodlights.</p&gt <p&gt<strong&gtSmart Sensor Mode</strong&gt HS-FLS100+ can be installed and used simply as an outdoor motion and LUX sensor. Floodlight functionality is not required. This provides some interesting security and smart home possibilities!</p&gt

### Inclusion Information

<p&gtPress the Link button 3 times within 1.5 seconds to put the unit into inclusion mode.</p&gt

### Exclusion Information

<p&gtPress the Link button 3 times within 1.5 seconds to put the unit into exclusion mode.</p&gt
...
<p&gtRange: 8~720 secs</p&gt <p&gtDefault: 15</p&gt
...
<p&gtFactory default at 50</p&gt <p&gtSetting = 0: always don’t turn ON light</p&gt <p&gtSetting = 30~200: lighting activation by setting</p&gt <p&gtSetting = 255: turn ON light upon PIR triggered</p&gt
...
<p&gtDefault: 10</p&gt <p&gtSetting: 0~1440 (min.)</p&gt
...
<p&gtPeriod to send Trigger Off command after PIR is triggered. Before this period expires, the PIR will not be able to detect any subsequent motion.</p&gt
...
<p&gtWhen the lux level falls below this threshold and the PIR gets triggered, the unit emits a Basic Set Command (Value=0xFF) and turns on its floodlight. This overwrites the Lux level set by the Lux knob.</p&gt <p&gtSetting = 0: always don’t turn ON light</p&gt <p&gtSetting = 30~200: lighting activation by setting</p&gt <p&gtSetting = 255: turn ON light upon PIR triggered</p&gt <p&gtIf the level is set to 0, the unit will not turn on its floodlight. The PIR will still send a notification report to the controller; if the level is set to 255, when motion is sensed, the sensor ignore the ambient brightness but just turn on the light.</p&gt
...
<p&gtDevice Reset Locally Notification</p&gt <p&gtNotification Report </p&gt <p&gtBinary Report </p&gt <p&gtSensor Multilevel Report </p&gt <p&gtBasic Report</p&gt
