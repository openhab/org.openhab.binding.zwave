...
<p&gtA configuration get command on this parameter returns the latest set parameter value (set by Z-wave).</p&gt <p&gtThis is a set only value, if changed locally on keypad these values are not changed on Z-wave module. Value 5, 6 and 7 are for future use on door lock.</p&gt
The following option values may be configured -:
