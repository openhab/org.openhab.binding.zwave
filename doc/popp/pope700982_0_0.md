...
<p&gtOnce installed the sensor will report open and close status changes to a central Z-Wave controller using notiﬁcation commands. Addtionally the sensor can directly control other device using the association group 2. Using conﬁguration commands the source of open and close events can choosen between the internal magnet detector or external dry contact connected via the screw terminal. The device is protected by a tamper switch.</p&gt
...
<p&gtThis parameter deﬁnes when the red LED will indicate events. Disabling all indications may extend battery life. Size: 1 Byte, Default Value: 7 (values 1 + 2 + 4 summarized)</p&gt
...
sent after a delay
<p&gtOn command is sent after a delay deﬁned in this parameter. Size: 2 Byte, Default Value: 0</p&gt
...
sent after a delay
<p&gtOff command is sent after a delay deﬁned in this parameter. Size: 2 Byte, Default Value: 0</p&gt
Values in the range 0 to 32400 may be set.
