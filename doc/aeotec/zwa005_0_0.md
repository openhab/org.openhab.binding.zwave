...
<p&gtAeotec TriSensor is a universal Z-Wave Plus compatible product, consists of temperature, lighting and motion sensors, powered by a CR123A battery. It can be included and operated in any Z-Wave network with other Z-Wave certified devices from other manufacturers and/or other applications. By the built-in motion sensor, an alarm will be sent to the gateway when the motion sensor is triggered.</p&gt

### Inclusion Information

<p&gtPress once TriSensor’s Action Button. If it is the first installation, the yellow LED will keep solid until whole network processing is complete. If successful, the LED will flash white -> green -> white -> green, after 2 seconds finished. If failed, the yellow LED lasts for 30 seconds, then the green LED flashes once. If it is the S2 encryption network, please enter the first 5 digits of DSK.</p&gt

### Exclusion Information

<p&gtPress once TriSensor’s Action Button, the Purple LED will keep solid until whole network processing is complete. If the exclusion is successful, the LED will flash white -> green - >white -> green and then LED will pulse a blue. If failed, the yellow LED lasts for 30 seconds, then the green LED flashes once.</p&gt
...
<p&gtPress and hold the button at least 2s until Red Led is on and then release the button,device will send wakeup notification to controller if device is in a Z-Wave network.</p&gt

### General Usage Information

<h2&gt Factory reset</h2&gt <ol&gt<li&gtPower up the device.</li&gt <li&gtPress and hold the button for 15s until Red Led is blinking,then release the button.</li&gt </ol&gt<p&gtNote: Please use this procedure only when the network primary controller is missing or otherwise inoperable.</p&gt
...
<p&gtThis parameter is configured the delay time before PIR sensor can be triggered again to reset motion timeout counter.</p&gt
...
<p&gtThis parameter is configured the time to clear motion event after a motion event detected. Time to motion clear, the device will send a clear event report to controller and send BASIC_SET= 0x00 to nodes associated in group 2.</p&gt
...
<p&gtThis parameter is configured the sensitivity that motion detect.</p&gt
...
<p&gtThis parameter enables sending BASIC_SET command to nodes that associated in group 2 and group 3.</p&gt
The following option values may be configured -:
...
<p&gtThis parameter is configured the Led light on disable or enable.</p&gt
The following option values may be configured -:
...
<p&gtChange threshold value for change in temperature to induce an automatic report for temperature sensor. Setting of value 20 can be a change of -2.0 or +2.0 (C or F depending on Parameter No.20) to induce automatic report or setting a value of 2 will be a change of 0.2(C or F).</p&gt
...
<p&gtChange threshold value for change in light sensor to induce an automatic report for temperature sensor.</p&gt
...
<p&gtThis parameter is configured the time interval for temperature sensor report. This value is larger, the battery life is longer. And the temperature value changed is not obvious.</p&gt
Values in the range 0 to 32767 may be set.
...
<p&gtThis parameter is configured the time interval for light sensor report. This value is larger, the battery life is longer. And the light intensity changed is not obvious.</p&gt
Values in the range 0 to 32767 may be set.
...
<p&gtThe current measuring temperature value can be add and minus a value by this setting.</p&gt
...
<p&gtThe current measuring light intensity value can be add and minus a value by this setting.</p&gt
...
<p&gtThis configuration defines the calibrated scale for ambient light intensity. Because the method and position that the sensor mounted and the cover of sensor will bring measurement error, user can get more real light intensity by this parameter setting. User should run the steps as blows for calibrating:</p&gt <ol&gt<li&gtSet this parameter value to default (Assumes the sensor has been added in a Z- Wave Network).</li&gt <li&gtPlace a digital luxmeter close to sensor and keep the same direction, monitor the light intensity value (Vm) which report to controller and record it. The same time user should record the value (Vs) of luxmeter.</li&gt <li&gtThe scale calibration formula: k = Vm / Vs.</li&gt <li&gtThe value of k is then multiplied by 1024 and rounded to the nearest whole number.</li&gt <li&gtSet the value getting in 5) to this parameter, calibrate finished.</li&gt </ol&gt<p&gtFor example, Vm = 300, Vs = 2600, then k = 2600 / 300 = 8.6667,<br /&gtk = 8.6667 * 1024 = 8874.7 ≈ 8875. In this case, the parameter should be set to 8875.</p&gt
...
<p&gtGroup 2 is controlling group, all nodes associated in this group will be controlled through BASIC_SET command by the device when device detects a movement event. Basic Set Value is decided by Configuration Parameter 6.</p&gt

Association group 2 supports 5 nodes.

### Group 3: Temperature Alarm

<p&gtGroup 3 is controlling group, all nodes associated in this group will be controlled through BASIC_SET command by the device when device detects a movement event. Basic Set Value is decided by Configuration Parameter 6.</p&gt
