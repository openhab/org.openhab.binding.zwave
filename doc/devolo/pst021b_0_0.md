...
<p&gtThe Home Control Motion Sensor measures brightness and temperature. Detects movements in an angle of 80 degrees horizontally and in a distance of up to 8 metres. The integrated light sensor can also be an impulse transmitter for the light controls.</p&gt
...
<p&gtParameter to set the Sensitivity for the PIR (Passiv Infrared Sensor)</p&gt <p&gt0-> means disable the PIR motion;</p&gt <p&gt1 - 99-> means the lowest sensitivity, 99 means the highest sensitivity</p&gt
...
<p&gtSetting the illumination threshold to turn on the light. When the event triggered and the environment illumination lower then the threshold, the device will turn on the light</p&gt <p&gt0 ->  means turn off illumination detected function and never turn on the light</p&gt
...
Operation Mode
<p&gtBit 0: 1 means security mode; 0 means home automation mode. </p&gt <p&gtBit 1: 1 means enable test mode; 0 means disable test mode. Notice: Bit0 and Bit1 will effect when the DIP Switch setting to program mode. If Bit1 is enabled, the Bit0 is useless.</p&gt <p&gt Bit 2: Disable </p&gt
...
<p&gtBit 0: Disable magnetic integrate illumination. </p&gt <p&gtBit 1: Disable PIR integrate Illumination. </p&gt <p&gtBit 2: Disable magnetic integrate PIR (Default is Disable) </p&gt <p&gtBit 3: When Bit2 is 0 (Enable), the device is install in the same room with the light? 0: In the same room</p&gt
Values in the range 0 to 127 may be set.
...
<p&gtParameter to set the Customer Function. </p&gt <p&gtBit 0: Reserved. </p&gt <p&gtBit 1: Enable sending motion OFF report. 0: Disable, 1: Enable. Note: Depends on the Bit4, 0: Report Notification CC, Type: 0x07, Event: 0xFE 1: Sensor Binary Report,</p&gt
...
<p&gtIn the security mode, after the PIR motion detected, setting the re-detect time </p&gt
...
<p&gtAfter turn on the light, setting the delay time to turn off the light when the PIR motion is not detected</p&gt
