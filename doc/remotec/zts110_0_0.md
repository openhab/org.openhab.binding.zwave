...
<p&gtZTS-110 Z-Thermostat is a Z-Wave enabled programmable thermostat that allows you to control your room temperature with programmable time schedule such as WAKE, AWAY, HOME and SLEEP event which can maximize energy conservation and comfort while minimizing the effort required to maintaining the appropriate temperature in your home whether you are at home or away.</p&gt <p&gtAlso, you can use the ZTS-110 to control / check your room temperature by smart phone or PC while you are at home or outside through Z-Wave gateway.</p&gt <p&gtRead following scenarios carefully before you start as it matters to the battery life under Z-Wave operation:</p&gt <p&gtZTS-110 can be powered by 4 x AA batteries, and/or 24Vac C wire.</p&gt <ol&gt<li&gtIf it is powered by batteries or powered by batteries first then applied with 24Vac before Z-Wave inclusion, ZTS-110 will self-configure to FLiRS mode which will save battery life by sleeping.</li&gt <li&gtIf it is powered by 24Vac or powered by 24Vac first then applied with batteries before Z-Wave inclusion, ZTS-110 will self-configure to Always Listening Mode which will not sleep.</li&gt <li&gtAfter inclusion process, ZTS-110 will not detect power source and not allow changing operation mode. You must perform exclusion process first if need to change Z-Wave operation mode.</li&gt <li&gtAfter Z-Wave inclusion process, if you reset ZTS-110 to default while both 24Vac and batteries are applied, ZTS-110 will take 24Vac as primary power source and self-configure to Always Listening Mode, because reset to default process will automatically exclude ZTS-110 from the Z-Wave network. You should disconnect the power source and re-apply the power so ZTS-110 can detect the power source type and self-configure to corresponding mode.</li&gt </ol&gt

### Inclusion Information

<ol&gt<li&gtPress and hold "Home" key for 2 seconds to set ZTS-110 to Add (Inclusion) / Delete (Exclusion) Mode.</li&gt <li&gtPress "Prog" key once, it will search the network. </li&gt </ol&gt
...
<p&gtDetermines Upper Limit of set point A. Unit in celsius A=(B+2)min (37.0 C)max, Unit in Fahrenheit A=(B+4)min (99.0 F)max  </p&gt
...
<p&gtDetermines Lower Limit of set point B. Unit in celsius (5.0 C to 35.0 C) Unit in Fahrenheit (41.0 F to 95.0  F)  </p&gt
...
<p&gtThermostat sends 'Basic Set' command to its association node repeatedly ever X minutes.</p&gt <ul&gt<li&gt0 = Disabled or</li&gt <li&gtSet to Enabled Values available 3 to 255.  </li&gt </ul&gt
...
<p&gtTrigger AUTO report if room temperature is different from last report.</p&gt <ul&gt<li&gt0 = Disabled or</li&gt <li&gtset value from 1 to 8 where each number corresponds to 1 Degree F or .5 Degrees Celsius.  </li&gt </ul&gt
...
<ul&gt<li&gt0 = Disabled or</li&gt <li&gtset value from 1 to 10 in 1 degree F increments or -1 to -10 in 1 degree F increments . </li&gt </ul&gt
...
<ul&gt<li&gt0 = Disabled or</li&gt <li&gtset value from 1 to 16 where 1 = .5 hours and each consecutive number increases time by .5 hours. </li&gt </ul&gt
