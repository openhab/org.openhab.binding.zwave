...
<p&gtRemotely controlled Dimmer designed to operate with or without neutral lead. It may be used as a switch in systems without neutral lead.</p&gt <p&gtAs a Dimmer it operates under the following loads:</p&gt <ul&gt<li&gtconventional incandescent</li&gt <li&gtHalogen 230V</li&gt <li&gtlow voltage halogen 12V (with electronic transformers)</li&gt <li&gtdimmable LED</li&gt <li&gtwhen used with FGB001 it may operate with any dimmable load up to 500W</li&gt </ul&gt

### Inclusion Information

<p&gtThe Dimmer enters the mode of inclusion after a quick pressing three times the push-button connected to S1 terminal or push-button B located inside the housing. For bi-stable switch perform 3 position changes.</p&gt

### Exclusion Information

<p&gtThe Dimmer enters the mode of exclusion after a quick pressing three times the push-button connected to S1 terminal or push-button B located inside the housing. For bi-stable switch perform 3 position changes</p&gt
...
<p&gtInfo: Key no. 2 is not represented by any physical device expect of devices on association list. This functionality prevents of lack of reaction on pressing key no. 2 through polling devices from association list one by one and checking their actual states</p&gt
The following option values may be configured -:
...
<p&gtTime of MANUALLY moving between the extreme dimming values</p&gt
...
<p&gtOptions for changing parameter 1-255 (10ms - 2,5s) - 0: this value disables the smooth change in light intensity. NOTE: value 0 is required for inductive and capacitive devices unsuitable for dimming (e.g. fluorescent lamps, motors, etc...)</p&gt
...
<p&gtThe function of 3-way switch provides the option to double key no. 1. The dimmer may control two bi-stable push-buttons or an infinite number of mono-stable push-buttons. (default value 0)</p&gt
The following option values may be configured -:
...
<p&gtThis function will enable decreasing the minimum level of the Dimmer by extending the control impulse.[100 - 170].</p&gt <p&gtBy changing the minimem level, the user may completely dim LED bulbs. Not all LED bulbs available on the market have the dimming capabilty.</p&gt
