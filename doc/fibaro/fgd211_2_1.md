...
<p&gtRemotely controlled Dimmer is designed to operate with or without neutral lead. It may be used as a switch in systems without neutral lead.</p&gt

### Inclusion Information

<p&gtThe Dimmer enters the mode of inclusion after quick pressing three times the push-button connected to S1 terminal or push-button B located inside the housing.</p&gt

### Exclusion Information

<p&gtThe Dimmer enters the mode of exclusion after quick pressing three times the push-button connected to S1 terminal or push-button B located inside the housing.</p&gt
...
<p&gtSending commands to control devices assigned to 1-st association group (key no. 1).</p&gt <p&gtNOTE: Parameter 15 value must be set to 1 to work properly. This activates the double-click functionality - dimmer/roller shutter control.</p&gt
The following option values may be configured -:
...
<p&gtInfo: Key no. 2 is not represented by any physical device expect of devices on association list. This functionality prevents of lack of reaction on pressing key no. 2 through polling devices from list one by one and checking thier actual states.</p&gt <p&gtIt is not possible to check the device status before sending a control frame from the key no. 2 if roller blind switch is chosen in parameter 14 (value 2)</p&gt <p&gtIf devices state is checked before sending asociation then parameter 19 should be set to value 0.</p&gt
The following option values may be configured -:
...
<p&gtOptions for changing parameter 0-255 (0ms – 2.5s)</p&gt <p&gt0 - this value disables the smooth change in light intensity</p&gt <p&gtNOTE value 0 is required for inductive and capacitive devices unsuitable for dimming, (e.g. fluorescent lamps , motors etc.)</p&gt
...
<p&gtThe function of 3 - way switch, provides the option to double key no. 1.<br /&gtThe Dimmer may control two toggle push-buttons or an infinite number of momentary push-buttons.  </p&gt
The following option values may be configured -:
...
<p&gtInfo: Remote control from Fibaro System Is Still Possible. This function is useful When you want display status of external devices, e.g. Motion Sensor, in Fibaro System.  </p&gt
The following option values may be configured -:
...
<p&gtThis function will enable decreasing the minimum level of the Dimmer by extending the control impulse. [100 - 170]      </p&gt <p&gtBy changing the minimem level, the user may completely dim LED bulbs.</p&gt <p&gtNot all LED bulbs available on the market have the dimmm</p&gt
...
<p&gtAlarm of any type (general alarm, water flooding alarm, smoke alarm: CO, CO2, temperature alarm).  </p&gt
The following option values may be configured -:
...
<p&gtParameter’s value changes depending on the dimming level.  </p&gt
Values in the range 1 to 99 may be set.
