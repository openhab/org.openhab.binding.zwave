...
<p&gtRemotely Controlled Relay Switch of Fibaro System is designed to operate in a wall switch box or in locations where the control of an electric device is needed.</p&gt <p&gtIt is also possible to send a signal to any system that is to be integrated with Fibaro system.</p&gt
...
<p&gtAutomatic turning off relay 1 after set time, in 100ms increments: 0=disabled (default), 65535=6535,5 secs</p&gt
...
<p&gtEnable/Disable operation of dimmer or roller shutter devices associated to group 1.</p&gt <p&gtAvailable only when using mono-stable inputs (buttons) - Hold button 1 or double-tap for operation</p&gt
The following option values may be configured -:
