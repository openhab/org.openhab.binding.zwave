...
<p&gtRemotely Controlled Relay Switch of Fibaro System is designed to operate in a wall switch box or in locations where the control of an electric device is needed.</p&gt
...
<p&gtIf Parameter 4 states an auto off period, this parameter enable the possibility to turn off the relay with a button push. Default value 0 disallow turn off with a button push during the auto off interval.</p&gt
The following option values may be configured -:
...
<p&gtDefault: 0 (Auto off disabled). In 100ms increments.</p&gt
...
<p&gtto 1-st association group (key no. 1)</p&gt <p&gt- Also see param #15</p&gt
The following option values may be configured -:
...
<p&gtAvailable configuration parameters:<br /&gt0 - [On / Off] device changes status on key status change.<br /&gt1 - Device status depends on key status: ON when the key is ON, OFF when the key is OFF </p&gt
The following option values may be configured -:
...
<p&gtenabling this option allows the user to dim lighting/shut roller by associating Dimmer/Roller Shutter Controller and holding or double press of double switch (only mono-stable switch). </p&gt
The following option values may be configured -:
