...
<p&gtKey no.2 is not represented by any physical device - only devices in the association list.</p&gt <p&gtThis functionality prevents of lack of reaction on pressing key no.2 through polling devices from association list one by one and checking their</p&gt
The following option values may be configured -:
...
<p&gtThis function will enable decreasing the minimum level of the Dimmer by extending the control impulse.[100 - 170]</p&gt <p&gtBy changing the minimem level, the user may completely dim LED bulbs.</p&gt <p&gtNot all LED bulbs available on the market support this.</p&gt
...
<p&gtAvailable settings: from 1 to 99. Parameter’s value changes depending on the dimming level.</p&gt
Values in the range 0 to 255 may be set.
