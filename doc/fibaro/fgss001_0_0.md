...
<p&gtThe Fibaro Smoke Sensor is a universal, optical Z-Wave smoke sensor. The device can be hard wired (12 or 24 VDC) or battery operated (battery life 3 years*). Smoke detection is signaled through siren and LED diode blinking. Additionally, the smoke sensor signal can be sent to an alarm system or fire alarm system hub, through a NC (normally closed) contact terminals. </p&gt <p&gtThe optical sensor detects smoke at an early stage of fire, often before flames appear and temperature starts to rise significantly. Moreover the device has a built-in temperature sensor, which can also trigger the alarm once the specified temperatute threshold is exceeded. </p&gt <p&gtThe Fibaro Smoke Sensor is designed to be placed on a wall or ceiling. LED indicator signals fire, operating mode and used to see if device is within the Z-Wave network. The smoke sensor is designed to operate in confined spaces, under normal conditions (lacking smoke, dust, condensed water vapor)</p&gt

### Inclusion Information

<p&gtManual Fibaro Smoke Sensor inclusion:</p&gt <ol&gt<li&gtConnect the power source to the Fibaro Smoke Detctor and/or install the battery. Make sure the device is located within direct range of the Z-Wave controller.</li&gt <li&gtSet the main controller into the learn mode (see main controller’s operating manual).</li&gt <li&gtQuickly, triple click the B-button, located on Fibaro Smoke Sensor’s enclosure.</li&gt <li&gtFibaro Smoke Sensor will be detected and included in the Z-Wave network.</li&gt </ol&gt

### Exclusion Information

<ol&gt<li&gtMake sure the sensor is connected to power source (battery and/or hard wired).</li&gt <li&gtSet the main controller into the learn mode (see main controller’s operating manual)</li&gt <li&gtQuickly, triple click the B-button, located on Fibaro Smoke Sensor’s enclosure</li&gt </ol&gt
...
<p&gtIf the sensor is already included in the Z-Wave network, wake it up by triple clicking the B-button..</p&gt
...
<p&gtparameter allows for LED indicator and acoustic alarm deactivation in case of smoke detection</p&gt
...
<p&gtAllows for choosing a command class used in 1-st Association Group.</p&gt
The following option values may be configured -:
...
<p&gtValue of the parameter specifying the forced level of dimming / opening sun blinds when comes 'switch on' / 'open' command to devices from association group no. 1</p&gt <p&gtIn the case of alarm frames the alarm priority is specified.</p&gt
...
<p&gtInterval between successive readings of temperature from all sensors connected to the device in seconds.</p&gt <p&gtNote: taking temperature readings from the sensor does not result in sending a temperature condition report to the central hub.</p&gt
...
<p&gtTransmitting the alarm or control frame in 'broadcast' mode (i.e. to all devices within range), information sent in this mode is not repeated by the mesh network.</p&gt <p&gt<strong&gtNOTE</strong&gt: If the broadcast mode of information transmission is activated for a given </p&gt
The following option values may be configured -:
...
<p&gtTemperature value to be added to or deducted from the current measured temperature measured by internal temperature sensor in order to compensate the difference between air temperature and temperature measured at the ceiling level</p&gt <p&gtAvailable settings: -1000 -/+ 1000 (in 0,1 C steps)</p&gt
...
<p&gtTime period between the consecutive Z-Wave network range tests.</p&gt <p&gtAvailable settings: 0 – 144 (10min multiplier) [10min-24h]</p&gt <p&gt0 – Z-Wave network range test inactive Default setting: 1 (10min)</p&gt
...
<p&gtTemperature value, measured by the built-in temperature sensor which the temperature alarm is sent.</p&gt <p&gtAvailable settings: 0,2 - 100</p&gt <ul&gt<li&gt0 – temperature alarm inactive</li&gt <li&gt2 - 100 – (2 - 100 C) Default setting:</li&gt <li&gt54 (54 C)</li&gt </ul&gt
Values in the range 0 to 100 may be set.
...
<p&gtThere are 3 levels of sensitivity to smoke presence.</p&gt <p&gtLevel 1 means the highest sensitivity.</p&gt
The following option values may be configured -:
...
<p&gtParameter specifies temperature and smoke level after which the Black Box starts recording them.</p&gt
The following option values may be configured -:
...
Malfunction alarm
<p&gtTime interval in which malfuntion alarm, if detected, is repeated using visual and acoustic alarms.</p&gt <p&gtAvailable settings:</p&gt <ul&gt<li&gt1 - 255 (100 ms multiplier) [100ms-25,5s]</li&gt <li&gtDefault setting: 10 (1s) </li&gt </ul&gt
...
Temperature alarm
<p&gtTime interval in which temperature alarm, if detected, is repeated using visual and acoustic alarms.</p&gt <p&gtAvailable settings:</p&gt <ul&gt<li&gt1 - 255 (100 ms multiplier) [100ms-25,5s]</li&gt <li&gtDefault setting: 5 (500ms)</li&gt </ul&gt
Values in the range -126 to 127 may be set.
...
<p&gtTime interval in which lack of the Z-Wave network alarm, if detected, is repeated using visual and acoustic alarms.</p&gt <p&gtAvailable settings:</p&gt <ul&gt<li&gt1 - 17280 (5s multiplier) [5s-24h]</li&gt <li&gtDefault setting: 360 (30min)</li&gt </ul&gt
...
Low battery alarm
<p&gtTime interval in which low battery alarm, if detected, is repeated using visual and acoustic alarms.</p&gt <p&gtAvailable settings:</p&gt <ul&gt<li&gt1 - 17280 (5s multiplier) [5s-24h]</li&gt <li&gtDefault setting: 360 (30min)</li&gt </ul&gt
Values in the range 1 to 17280 may be set.
...
<p&gtConsider temperature measurement compensation (parameter 73) when sending temperature report.</p&gt
The following option values may be configured -:
