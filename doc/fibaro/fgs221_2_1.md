...
<p&gtRemotely Controlled Relay Switch of Fibaro System is designed to operate in a wall switch box or in locations where the control of an electric device (up to 3kW) is needed.<br /&gtIt is also possible to send a signal to any system that is to be integrated with Fibaro system.</p&gt

### Inclusion Information

<p&gtAdd Fibaro Switch to the network by pushing three times key no. 1 or push-button B located inside the housing of the device. For toggle switch perform 3 position changes.</p&gt

### Exclusion Information

<p&gtRemove Fibaro Switch from the network by pushing three times key no. 1 or push-button B located inside the housing of the device. For toggle switch perform 3 position changes.</p&gt
...
<p&gtAuto off relay after specified time, with the possibility of manual override - immediate Off after button push.</p&gt
The following option values may be configured -:
...
<p&gtSending commands to control devices assigned to 1-st association group (key no. 1)<br /&gtNOTE: Parameter 15 value must be set to 1 to work properly. This activates the double-click functionality - dimmer/roller shutter control</p&gt
The following option values may be configured -:
...
<p&gtSending commands to control devices assigned to 2-nd association group (key no. 2).  </p&gt <p&gtInfo: Parameter 15 must be set to 1 to make this parameter work properly. This activates the double-click functionality - dimmer/roller shutter control.</p&gt
The following option values may be configured -:
...
<p&gtOperation of the Dimmer and Roller Shutter Controller - enabling this option allows the user to dim lighting/shut roller by associating dimmer/Roller Shutter Controller and holding or double press of double switch (only mono-stable switch).  </p&gt
The following option values may be configured -:
...
<p&gtFibaro Switch returns to the last position saved before a power failure</p&gt <p&gtOptions for changing the parameter 0-1</p&gt
The following option values may be configured -:
