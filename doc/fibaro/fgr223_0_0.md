...
<p&gtFIBARO Roller Shutter 3 is a device designed to control roller blinds, awnings, venetian blinds, gates and other single phase, AC powered devices.</p&gt <p&gtRoller Shutter 3 allows precise positioning of roller blinds or venetian blind lamellas. The device is equipped with power and energy monitoring. It allows to control connected devices either via the Z-Wave network or via a switch connected directly to it.</p&gt

### Inclusion Information

<p&gtAdding (Inclusion) – Z-Wave device learning mode, allowing to add the device to existing Z-Wave network.</p&gt <p&gtTo add the device to the Z-Wave network using Smart Start:</p&gt <ol&gt<li&gtSet the main controller in Security S2 Authenticated add mode (see the controller’s manual).</li&gt <li&gtScan the DSK QR code or input the underlined part of the DSK (label on the bottom of the box).<br /&gt(on the device label).</li&gt <li&gtPower the device (turn on the mains voltage).</li&gt <li&gtLED will start blinking yellow, wait for the adding process to end.</li&gt <li&gtSuccessful adding will be confirmed by the Z-Wave controller’s message.</li&gt </ol&gt<p&gtTo add the device to the Z-Wave network manually:</p&gt <ol&gt<li&gtPower the device.</li&gt <li&gtIdentify the S1 switch.</li&gt <li&gtSet the main controller in (Security/non-Security Mode) add mode (see the controller’s manual).</li&gt <li&gtQuickly, triple click the S1 switch.</li&gt <li&gtIf you are adding in Security S2, scan the DSK QR code or input the underlined part of the DSK (label on the bottom of the box)</li&gt <li&gtWait for the adding process to end.</li&gt <li&gtSuccessful adding will be confirmed by the Z-Wave controller’s message.</li&gt </ol&gt<p&gtIn case of problems with adding using the S1 switch, use B-button instead (located on the housing).<br /&gtSmartStart enabled products can be added to SmartStart enabled Z-Wave controller by scanning the Z-Wave QR Code present on the product. SmartStart product will be added automatically within 10 minutes of being switched on in the network range.</p&gt

### Exclusion Information

<p&gtRemoving (Exclusion) – Z-Wave device learning mode, allowing to remove the device from existing Z-Wave network. Removing the device restores all the default parameters of the device, but does not reset power metering data.</p&gt <p&gtTo remove the device from the Z-Wave network:</p&gt <ol&gt<li&gtMake sure the device is powered.</li&gt <li&gtIdentify the S1 switch.</li&gt <li&gtSet the main controller in remove mode (see the controller’s manual).</li&gt <li&gtQuickly, triple click the S1 switch.</li&gt <li&gtWait for the removing process to end.</li&gt <li&gtSuccessful removing will be confirmed by the Z-Wave controller’s message.</li&gt </ol&gt<p&gtIn case of problems with removing using the S1 switch, use B-button instead (located on the housing).</p&gt <p&gtIf parameter 40 enables scenes for triple S1 click, disable it or use B-button instead to remove the device.</p&gt <p&gtIf parameter 24 is set to 1, use S2 switch instead to remove the device.</p&gt
...
Switch type
<p&gtThis parameter defines as what type the device should treat the switch connected to the S1 and S2 terminals.</p&gt <p&gtThis parameter is not relevant in gate operating modes (parameter 151 set to 3 or 4). In this case switch always works as a momentary and has to be connected to S1 terminal.</p&gt <p&gtIf parameter 20 is set to 1 (toggle switch), change value of pa- rameter 153 to 0 for slats to work properly.</p&gt
...
Inputs orientation
<p&gtThis parameter allows reversing the operation of switches connected to S1 and S2 without changing the wiring.</p&gt
The following option values may be configured -:
...
Outputs orientation
<p&gtThis parameter allows reversing the operation of Q1 and Q2 without changing the wiring (in case of invalid motor connection) to ensure proper operation.</p&gt
The following option values may be configured -:
...
Force calibration
<p&gtBy setting this parameter to 2 the device enters the calibration mode. The parameter relevant only if the device is set to work in positioning mode (parameter 151 set to 1, 2 or 4).</p&gt
The following option values may be configured -:
...
Operating mode
<p&gtThis parameter allows adjusting operation according to the connect- ed device.</p&gt
The following option values may be configured -:
...
<p&gtFor Venetian blinds (parameter 151 set to 2) the parameter determines slats positioning in various situations.<br /&gtThe parameter is irrelevant for other modes.<br /&gtIf parameter 20 is set to 1 (toggle switch), change value of pa- rameter 153 to 0 for slats to work properly.</p&gt
The following option values may be configured -:
...
<p&gt"Roller Shutter" is assigned to shutter position control – UP/DOWN (uses Switch Multilevel command class).</p&gt
...
<p&gt"Slats" is assigned to slats position control – UP/DOWN (uses Switch Multilevel command class).</p&gt
