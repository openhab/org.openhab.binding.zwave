...
<p&gtThe Fibaro Door / Window Sensor is a wireless, battery powered, Z-Wave compatible reed sensor. Each time its two parts, i.e. the sensor's body and a magnet separate, a radio signal is sent. In addition the Fibaro Door / Window Sensor supports one DS18B20 temperature sensor and has one potential free input. The Fibaro Door / Window Sensor is designed for use with scenes in home automation systems and everywhere else where information related to opening / closing of doors, windows, garage gates, etc. is needed.</p&gt

### Inclusion Information

<p&gt<strong&gtPrepare the sensor:</strong&gt</p&gt <ol&gt<li&gtOpen the sensor with a small screwdriver (be careful)</li&gt <li&gtRemove the paper between the battery an the contact to activate the sensor.</li&gt <li&gtClose the sensor (be sure that it is closed correctly. The sensor must not act like a button if you push the casing. This sometimes happens. It is important to close the device correctly.)</li&gt </ol&gt<p&gt<strong&gtInclude the sensor in the network:</strong&gt</p&gt <ol&gt<li&gtStart HABmin.</li&gt <li&gtNavigate to Configuration > Bindings > select the ZWAVE Binding > select the devices tab on the right</li&gt <li&gtYou should see at least your ZWAVE-Controller in the list of devices.</li&gt <li&gtHit "include" to start the 30 sec include mode</li&gt <li&gtHit the button on the back of the sensor 3 times in a short row.</li&gt <li&gtThe device starts to blink in blue. If it stops repeat step 5 until the 30 sec inclusion is over. Keep the sensor not to close and not too far from the controller (between 30 cm and 100 cm)</li&gt <li&gtHit "reload properties" in HABmin.</li&gt <li&gtThe new device should appear in the list (grey with no name)</li&gt <li&gtIf not, hit reload in your browser and navigate back to the device list.</li&gt <li&gtThe new device should appear in the list (grey with no name)</li&gt <li&gtIf not, restart your openHAB server, hit reload in your browser and navigate back to the device list.</li&gt <li&gtThe new device should appear in the list (grey with no name)</li&gt <li&gtIf not, repeat from step 4 until the device appears (This should happen rarely. I once needed two tries. Iadded 10 devices all together.)</li&gt <li&gtHit the button on the back of the sensor 3 times in a short row.</li&gt <li&gtThe device starts to blink in blue. Keep the sensor not to close and not too far from the controller (between 30 cm and 100 cm)</li&gt <li&gtHit "reload properties" in HABmin.</li&gt <li&gtRepeat steps 14 - 16 several times. The device in the list will first show up a name, then turn to yellow and then to green.</li&gt <li&gtCongratulations, you fully added the sensor and it should work correctly now.</li&gt </ol&gt

### Exclusion Information

<p&gt<strong&gtExclude the sensor from the network:</strong&gt</p&gt <ol&gt<li&gtStart exclude mode on the ZWAVE Controller.</li&gt <li&gtHit the button on the back of the sensor 3 times in a short row.</li&gt </ol&gt
...
<p&gtThe default wake-up interval will drain the battery in a few weeks to months. It's advised to set a minimum wake-up interval of 3 hours to 24 hours after the device is configured correctly and a node.xml file has been created for the device.</p&gt
...
Type of IN input
<p&gtDefault = 0- INPUT\_NC (Normally closed)</p&gt <p&gtAvailable parameter settings</p&gt <p&gt0 - INPUT\_NC (Normally closed)</p&gt <p&gt1 - INPUT\_NO (Normally open) </p&gt <p&gt2 - INPUT\_MONOSTABLE</p&gt <p&gt3 - INPUT_BISTABLE</p&gt
The following option values may be configured -:
...
<p&gtType of transmitted control frame for association group 1, activated via input IN1. The parameter allows to specify the type of alarm frame or to force transmission of control commands (BASIC\_SET)</p&gt <p&gtDefault value = 255 - BASIC SET</p&gt <p&gtAvailable parameter settings</p&gt <p&gt1 - ALARM SMOKE frame</p&gt <p&gt2 - ALARM CO frame</p&gt <p&gt3 - ALARM CO2 frame</p&gt <p&gt4 - ALARM HEAT frame</p&gt <p&gt5 - ALARM WATER frame</p&gt <p&gt255 Control frame BASIC\_SET</p&gt
The following option values may be configured -:
...
<p&gtDeactivating transmission of the alarm cancelling frame or the control frame deactivating the device (Basic). It allows for disabling the function of deactivating the device and cancelling alarms for devices associated with IN input. Default value: 0</p&gt <p&gtAvailable parameter settings:</p&gt <p&gt0 – for association group no. 1 information is sent</p&gt <p&gt1 – for association group no. 1 information is not sent.</p&gt <p&gtParameter size: 1 [byte].</p&gt
The following option values may be configured -:
...
<p&gtSensitivity to temperature changes. The maximum acceptable difference between the last reported temperature and the current temperature read from the sensor. If the temperatures differ by the set value or more, a report with the current temperature value is sent to the device assigned to association group no. 3.</p&gt <p&gtDefault value: 8 \[0,5o C]</p&gt <p&gtAvailable parameter settings: 0 – 255 [0o C to 16o C\] \[32o F – 60,8o F\] To set an appropriate parameter value use the following formula: x = delta T x 16 - for Celsius x = delta T x 80 / 9 - for Fahrenheit x = parameter value delta T – maximum acceptable temperature gradient in Celsius or Fahrenheit If the value is set to 0: - if the wake-up interval is set to 255 seconds, temperature report will be sent according to the interval, - if the wake-up interval is set to over 255, temperature report will be sent each ca. 4 minutes. Parameter size: 1 [byte].</p&gt
...
<p&gtSending an alarm or control frame (for IN input, depending on parameter no.5 value), and TMP button alarm frame. The frame is sent in “broadcast” mode, i.e. to all devices within range - information sent in this mode is not repeated by the mesh network.</p&gt <p&gtDefault value: 0</p&gt <p&gtAvailable parameter settings: 0 – IN and TMP Broadcast mode inactive,</p&gt <p&gt1 – IN broadcast mode active, TMP broadcast mode inactive</p&gt <p&gt2 – IN broadcast mode inactive, TMP broadcast mode active</p&gt <p&gt3 – IN and TMP broadcast mode active</p&gt <p&gtParameter size: 1 [byte].</p&gt
The following option values may be configured -:
