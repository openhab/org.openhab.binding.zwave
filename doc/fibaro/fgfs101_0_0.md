...
<p&gtFibaro Flood Sensor is a universal flood and temperature sensor. Device can be battery (ca. 2 years battery life) or VDC powered (12 or 24 VDC).</p&gt

### Inclusion Information

<p&gtConnect a DC power source to include the Fibaro Flood Sensor into the Z-Wave network in autoinclusion mode or triple click TMP button for manual inclusion.</p&gt

### Exclusion Information

<p&gtTriple click TMP button</p&gt
...
<p&gtTime period (in seconds) that an alarm will still be indicated after the alarm condition has ceased.</p&gt
...
<p&gtValue of the parameter specifying the forced level of dimming / opening sun blinds when comes 'switch on' / 'open' command to devices from association group 1.</p&gt
...
<p&gtSet to 0 to disable all-clear basic set & alarm frame generation.  After flooding, device will keep reporting flooding even if flooding ceased.</p&gt
The following option values may be configured -:
...
<p&gtInterval between successive readings of temperature in seconds.</p&gt
...
<p&gtTransmit the alarm or control frame in 'broadcast' mode.  Information sent in this mode is not repeated by the mesh network.</p&gt <p&gt<strong&gtNOTE:</strong&gt If the broadcast mode of information transmission is activated for a given alarm, then the information <strong&gtis not</strong&gt relayed through the mesh network.  It is only heard by directly reachable devices.</p&gt
The following option values may be configured -:
...
<p&gtThe parameter stores a temperature value (def. 15C), below which LED indicator blinks with a colour determined by a Parameter 61 settings. By default the LED indicator blinks blue.</p&gt <p&gtNegative numbers are encoded in U2 (two's compliment) standard.</p&gt <ul&gt<li&gt-1 actual temperature = 65535 reported</li&gt <li&gt-10000 = 55536</li&gt <li&gt-32768 = 32768</li&gt <li&gt32767 = 32767</li&gt </ul&gt
...
<p&gtThe parameter stores a temperature value (def. 35C), above which LED indicator blinks with a colour determined by the Parameter 62 settings. By default the LED indicator blinks red</p&gt
Values in the range -10000 to 10000 may be set.
...
<p&gt<strong&gtNOTE:</strong&gt A main controller interprets colours as a sum of it component colours value. Each colours value is a number from 0 to 255. Eg: Indicated colour = 65536 \* RED value + 256 \* GREEN value + BLUE value</p&gt
...
<p&gtParameter determines time period after which an alarm frame will be retransmitted. Value of 0 disables alarm frame retransmission.</p&gt <p&gt<strong&gtNOTE:</strong&gt In case the time period set in this parameter is shorter than the one specified in parameter 75, the device will not quiet the alarm; it will remain active.</p&gt
Values in the range 0 to 65535 may be set.
