...
<p&gtThe basic purpose of the device is to count impulses / flashes from a metering device and to save impulse count in nonvolatile memory with 15 minutes interval.</p&gt

### Inclusion Information

<p&gt3 times press on the button to include</p&gt

### Exclusion Information

<p&gt3 times press on the button to exclude</p&gt
...
<p&gtThis value also controls number of measurements per second on mechanical meters.</p&gt
...
<p&gtContains seconds * poll frequency before going back to sleep. If you press the button once this value will be set.</p&gt
Values in the range 0 to 65535 may be set.
...
<p&gtThe calibration progress is calculated as (value[1] - 65 + value[2] / value[3]) * 25.</p&gt <p&gtThe calibration is finished successfully when value[0]  =  2 and value[2] = value[3].</p&gt <p&gtThe calibration has failed if value[0] != 1 OR (value[0] = 2 AND value[2] != value[</p&gt
Values in the range 0 to 65535 may be set.
