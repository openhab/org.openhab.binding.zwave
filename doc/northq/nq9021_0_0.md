...
<p&gtA single click on the Z-Wave button will wake up the device</p&gt
...
IR Pulse Width
<p&gtAdjusts transmit signal so received signal will in average be equal to this values.</p&gt
Values in the range 0 to 255 may be set.
...
<p&gtWake device up this many times per second. This value also controls number of measurements per second on mechanical meters</p&gt
...
Real-time mode
<p&gtSet this to enter real-time mode from remote. Contains seconds * poll frequency before going back to sleep.</p&gt <p&gtIf you press the button once this value will be set.</p&gt
Values in the range 0 to 65535 may be set.
...
Calibration status
<p&gtThe calibration progress is calculated as (value[1] - 65 + value[2] / value[3]) * 25.</p&gt <p&gtThe calibration is finished successfully when value[0]  =  2 and value[2] = value[3].</p&gt <p&gtThe calibration has failed if value[0] != 1 OR (value[0] = 2 AND value[2] != value[</p&gtThis is an advanced parameter and will therefore not show in the user interface without entering advanced mode.
Values in the range 0 to 65535 may be set.
