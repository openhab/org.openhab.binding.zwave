...
<p&gtFlush 1D relay module responds to commands ALL ON / ALL OFF that may be sent by the main controller or by other controller belonging to the system.</p&gt
The following option values may be configured -:
...
<p&gtWhen relay is ON it goes automatically OFF after time defined by this parameter. Timer is reset to zero each time the module receive ON command regardless from where it comes (push button, associated module, controller,..).</p&gt
...
<p&gtWhen relay is OFF it goes automatically ON after time defined by this parameter. Timer is reset to zero each time the module receive OFF command regardless from where it comes (push button, associated module, controller,..).</p&gt
The following option values may be configured, in addition to values in the range 0 to 65535 -:
...
<p&gtAutomatic turning off / on seconds or milliseconds selection.</p&gt <p&gtNOTE: Parameter is the same for turning OFF or ON.</p&gt
The following option values may be configured -:
...
<p&gtSaving the state of the relay after a power failure</p&gt
The following option values may be configured -:
...
<p&gtEnabling I2 means that Endpoint (I2) will be present on UI. Disabling it will result in hiding the endpoint according to the parameter set value. Additionally, a Notification Type and Event can be selected for the endpoint.</p&gt <p&gt<strong&gtNOTE 1</strong&gt: After parameter change, first exclude module (without setting parameters to default value) then wait at least 30s and then re-include the module!</p&gt <p&gt<strong&gtNOTE 2</strong&gt: When the parameter is set to value 9 the notifications are sent for Home Security. </p&gt
The following option values may be configured -:
...
<p&gtSet value is added to or subtracted from actual measured value by sensor. </p&gt <p&gtAvailable configuration parameters:</p&gt <ul&gt<li&gt32536: offset is 0.0 °C</li&gt <li&gt1 to 100: value from 0.1 °C to 10.0 °C is added to actual measured temperature</li&gt <li&gt1001 to 1100: value from -0.1 °C to -10.0 °C is subtracted to actual measured temperature<br /&gt</li&gt </ul&gt
...
<p&gtIf digital temperature sensor is connected, module reports measured temperature on temperature change defined by this parameter.</p&gt <ul&gt<li&gt0: Reporting disabled</li&gt <li&gt1- 127: 0,1°C - 12,7°C, step is 0,1°C</li&gt </ul&gt
...
<p&gtLifeline group (reserved for communication with the main controller), 1 node allowed.</p&gt
...
Basic ON/OFF
<p&gtBasic ON/OFF (triggered at change of the output state and reflecting its state) up to 16 nodes.</p&gt
...
Basic ON/OFF (I2)
<p&gtBasic ON/OFF (triggered at change of the input I2 state and reflecting its state) up to 16 nodes. </p&gt
...
<p&gtBinary Sensor Report (triggered at change of the input I2 state and reflecting its state) up to 16 nodes.</p&gt
...
Notification Report
<p&gtTriggered at change of the input I2 state and reflecting its state) up to 16 nodes.</p&gt
...
<p&gtTriggered at change of temperature sensor up to 16 nodes.</p&gt
