...
Invert Switch
<p&gtChange the top of the switch to OFF and the bottom  of the switch to ON. </p&gt <p&gtNote: If you invert the switches and also install the product upside down, remember the load will now be controlled by the right, not the left switch.</p&gt
The following option values may be configured -:
...
<p&gtThis value indicates in 10 millisecond resolution, how often the dim level will change. </p&gt <p&gtFor example, if you set this parameter to 1,  then every 10ms the dim level will change. </p&gt <p&gtIf you set it to 255, then every 2.55 seconds the dim level will change.</p&gt
...
<p&gtThis value indicates in 10 millisecond resolution, how often the dim level will change. </p&gt <p&gtFor example, if you set this parameter to 1, then every 10ms the dim level will change. </p&gt <p&gtIf you set it to 255, then every 2.55 <br>seconds the dim level will change.</p&gt
Values in the range 1 to 255 may be set.
...
<p&gtThis value indicates in 10 millisecond resolution, how often the dim level will change.</p&gt <p&gt For example, if you set this parameter to 1, then every 10ms the dim level will change. </p&gt <p&gtIf you set it to 255, then every 2.55 seconds the dim level will change.</p&gt
Values in the range 1 to 255 may be set.
