...
<p&gt The device is excluded by triple click to one of the buttons.</p&gt
...
<p&gtTypical time used to differentiate click, hold, double and triple clicks<BR/>Defined in 10ms ticks.</p&gt
...
Switch by buttons
<p&gt If disabled, the local operations by buttons will not switch the load, but only send commands to On/Off association group. In this mode buttons are not linked with the switch</p&gt
The following option values may be configured -:
...
<p&gt Defines which command should be sent on button single press or hold. Basic and Scene Activation commands are sent to Association group. Use Scene Controller Conf to set up Scene ID for Scene Activation. Switch All commands are sent broadcast. </p&gt
The following option values may be configured -:
