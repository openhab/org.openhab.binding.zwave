 ...
 * </ul>
 * The responses required for each transaction are defined in the {@link SerialMessage} class and these are
 ...
 * device. Transactions are tracked using a callback ID.</li>
...
public class ZWaveTransactionManager {
    ...
    private static final int INITIAL_TX_QUEUE_SIZE = 20;
     ...
     */
    private final Map<Integer, PriorityBlockingQueue<ZWaveCommandClassTransactionPayload>> sendQueue = new HashMap<Integer, PriorityBlockingQueue<ZWaveCommandClassTransactionPayload>>();

    private final PriorityBlockingQueue<ZWaveTransaction> controllerQueue = new PriorityBlockingQueue<ZWaveTransaction>();

    private ZWaveTransaction outstandingDeviceTransaction = null;
    private ZWaveTransaction outstandingControllerTransaction = null;

    private final List<ZWaveCommandClassTransactionPayload> outstandingTransactions = new ArrayList<ZWaveCommandClassTransactionPayload>();
    ...
    /**
     * Add a controller transaction to the send queue.
     * This queues a {@link SerialMessage} to be sent to the controller only. The system will only allow one serial
     * message to be outstanding to the controller at once.
     *
     * @param transaction {@link ZWaveTransaction} to be sent
     */
    public void queueTransactionForSend(ZWaveTransaction transaction) {
        synchronized (controllerQueue) {
            ...
            if (controllerQueue.contains(transaction)) {
                ...
                controllerQueue.remove(transaction);
            }
    ...
    /**
     * Add device transaction to the send queue.
     * This queues a {@link ZWaveCommandClassPayload} to be sent to a device.
     *
     * @param transaction {@link ZWaveCommandClassTransactionPayload} to be sent
     */
    public void queueTransactionForSend(ZWaveCommandClassTransactionPayload transaction) {
        ...
        synchronized (sendQueue) {
            // The queue is a map containing a queue for each node
            // Check if this node is in the queue
            if (sendQueue.containsKey(transaction.getNodeId())) {
                // Now check if this transaction is already in the queue
                if (sendQueue.get(transaction.getNodeId()).contains(transaction)) {
                    // if (sendQueue.contains(transaction)) {
                    logger.debug("NODE {}: Transaction already on the send queue. Removing original.",
                            transaction.getNodeId());
                    sendQueue.get(transaction.getNodeId()).remove(transaction);
                }
            } else {
                // There's no queue for this node, so add it
                sendQueue.put(transaction.getNodeId(),
                        new PriorityBlockingQueue<ZWaveCommandClassTransactionPayload>(INITIAL_TX_QUEUE_SIZE));
                // new ZWaveCommandClassTransactionPayloadComparator());
            }

            // Add the message to the queue
            // We always add the most recent version, even though they are supposedly the same,
            // in case things like priority have changed
            sendQueue.get(transaction.getNodeId()).add(transaction);
        }
    ...
    public int getSendQueueLength() {
        int total = 0;
        for (PriorityBlockingQueue<ZWaveCommandClassTransactionPayload> queue : sendQueue.values()) {
            total += queue.size();
        }
     ...
     */
    public ZWaveCommandClassTransactionPayload getTransactionToSend() {
        ...
        synchronized (sendQueue) {
            for (int node : sendQueue.keySet()) {
                ...
                for (ZWaveCommandClassTransactionPayload outstandingTransaction : outstandingTransactions) {
                    if (outstandingTransaction.getNodeId() == node) {
                        outstanding = true;
                        break;
                    }
                }
            ...
            if (transaction != null) {
                sendQueue.get(transaction.getNodeId()).remove(transaction);
                if (sendQueue.get(transaction.getNodeId()).isEmpty()) {
                    sendQueue.remove(transaction.getNodeId());
                }
            }
        }
    ...
    public void processReceiveMessage(SerialMessage incomingMessage) {
        ...
        synchronized (transactionSync) {
            ...
            switch (incomingMessage.getMessageType()) {
                case Response:
                    // A RESponse is only ever received immediately after our request
                    // so this must be for our last sent message.
                ...
                case Request:
                    // If this is an ApplicationCommandMessage, then we try and correlate the command class data.
                    // Otherwise simply look at the callbackID
                    if (incomingMessage.getMessageClass() == SerialMessageClass.ApplicationCommandHandler) {
                        currentTransaction = transaction;
                    } else if (transaction.getCallbackId() != incomingMessage.getCallbackId()) {
                        currentTransaction = transaction;
                    }
                    break;
            }
    ...
    private void sendNextMessage() {
        ...
        synchronized (transactionSync) {
            ...
            final ZWaveCommandClassTransactionPayload payload = getTransactionToSend();
            if (payload == null) {
                // Nothing to send!
            ...
            // Add this to the outstandingTransactions list
            ...
            outstandingTransactions.add(payload);
