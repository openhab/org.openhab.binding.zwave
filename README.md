...
* Rebuild routing tables (nee Heal) - The node must be fully initialized to execute this action. This clears all current routes and establishes new ones for this device only. This action generates less network traffic (one node) than the network heal option on the controller page. The new situation for this node will be reflected in the Network Map. Battery nodes will execute this action when they are next awake.
* Ping the node (advanced) - Sends a ping to the node. If the node stays (or turns) ONLINE it has a network connection.
...
* Remove failed node from the controller (advanced) - Again for listening nodes only, after the controller has marked the node as failed, this will remove the node. To eliminate the Thing UI page after removal, use the Remove Thing at the very bottom of the page.
* Replace failed node with a new device, reusing the node ID (advanced) - For listening nodes, marked as failed this allows a new device to be added with the same node ID. After triggering the action, pause a second or two, then put the replacement device into inclusion mode. It is best to have debug logging on while using this action. If the replacement device is not the same, use Remove Thing for the current page and then go to Things > + sign in bottom right corner > Z-Wave Binding > Scan to get the correct UI page. The replacement device can be battery powered.
